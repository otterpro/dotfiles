...
"===========================================
" Neovim uses this to load vim packages in its path if we want to load both 
" neovim and vim's config
" for vim, it is not necessary
...
" --- fix color on windows terminal/cmd --------
" set color to 256 on Windows Terminal
" this fixes color but not sure how it affects linux side
set termguicolors
...
"--- tmux fix -- if in linux/mac desk terminal --------------
" Disable BCE (background-color-erase), because it causes color problem in " tmux
if &term =~ '256color'
	" =~ -> regex contains, since it could be term-256color or screen-256color
...
"Remember setting esp folded state -- doesn't work???
...
vmap '' S'
nmap <Leader>' ysiW'
" commented out below to preserve "go to previous jump"
" nmap '' ysiW'

imap "" <Esc>ysiW"Ea
vmap "" S"
nmap <Leader>" ysiW"
" TODO: comment out below? since we're no longer using nmap '', keep it consistent?
nmap ""  ysiW"
...
" vim-markdown: folding is enabled by default.    
" would disable due to performance issue, but so far so good?
